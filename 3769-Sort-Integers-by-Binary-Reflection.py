...
Sort the array in ascending order based on the binary reflection of each element. If two different numbers 
have the same binary reflection, the smaller original number should appear first.
...
class Solution(object):
    def sortByReflection(self, nums):
        return sorted(nums, key=lambda x: (int(bin(x)[:1:-1], 2), x))
        """
        :type nums: List[int]
        :rtype: List[int]
        """
